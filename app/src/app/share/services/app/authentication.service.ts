...
export class AuthenticationService {
   ...
   * - Actualiza signal
   */
  loginUser(credentials: { correo: string; contrasena: string }): Observable<{ token: string }> {
    return this.http
      .post<{ token: string }>(`${this.apiUrl}/usuarios/login`, credentials)
      .pipe(
        tap(({ token }) => {
  ...
  getUserProfile(): Observable<UsuarioModel | null> {
    return this.http.get<UsuarioModel>(`${this.apiUrl}/usuarios/profile`).pipe(
      tap((user) => {
